...
class P049_data_struct {
  ...
  bool init(const uint8_t serial_rx, const uint8_t serial_tx, bool setABCdisabled) {
    if (serial_rx < 0 || serial_tx < 0)
      return false;
    reset();
    easySerial = new SoftwareSerial(serial_rx, serial_tx);
    if (easySerial == nullptr) {
      return false;
    }
  ...
  SoftwareSerial *easySerial = nullptr;
